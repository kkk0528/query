...
public class Test2 {
    ...
    public List<CipherPub[]> zhan(List<CipherPub[]> A, CipherPub[] B, CP cp, CSP csp) {
        A.remove(0);
        A.set(0, B);
        CipherPub MAX[] = A.get(0);
        for (int i = 1; i < A.size(); i++) {
    ...
    public static void main(String[] args) {
        ...
        for (int n = 1; n < 2; n++) {
            ...
            Map<Integer, List> key_paixu1 = new LinkedHashMap<>();
            List PINGJIE_paixu = new ArrayList<>();
...
//            System.out.println(array_txtid);
//            System.out.println(ZONG_TEXT);
...
//                                    排序
            //        定义一个存放point key 分数的顺序的list
            //先从library取出一个关键字 例如“海景”
            //构造那张表
...
//                List<Double> tfidf_TEMP = new ArrayList<>();
...
//                List Z_DP_IDXY_TFIDF_minwen = new ArrayList<>();
                ArrayList<BigInteger> IDXY_sort_temp = new ArrayList<>();
//                ArrayList<CipherPub> TFIDF_PJ_sort_temp = new ArrayList<>();
...
//                    for (Map.Entry<Integer, Double> entry : point_lits_tmp.get(q).getKey_score().entrySet()) {
//                        tfidf_TEMP.add(entry.getValue());
//                    }
...
//        关键字匹配，取某一行，取前k个，分布式解密拆分（id|x|y|tfidf）
...
//                BigInteger ASDADADSDAA = paillierT.SDecryption(IDXY_IDXY);
...
//            for (int i = 0; i < k; i++) {
//                int temp = 0;
//                int re_temp1 = csp.slt_two(cp.SLT_one(E_ZONG.get(0)[2], ZONG.get(1).get(i)[2]));
//                BigInteger SADA =paillierT.SDecryption(E_ZONG.get(0)[2]);
//                BigInteger SASDA =paillierT.SDecryption(ZONG.get(1).get(0)[2]);
//                //k次比较re_temp如果为1，对E_ZONG进行操作，如果有一次为0或-1E_ZONG
//                if (re_temp1 == 1) {
//                    if (k==1){
//                        E_ZONG.remove(0);
//                        E_ZONG.add(0, ZONG.get(1).get(i));
//                    }
//                    else{
//                        for (int j = 1; j < E_ZONG.size(); j++) {
//                            int re_temp2 = csp.slt_two(cp.SLT_one(E_ZONG.get(j)[2], ZONG.get(1).get(i)[2]));
//                            if (re_temp2 == 0) {
//                                temp = temp - 100;
//                            } else {
//                                temp++;
...
//            System.out.println("k个保证栈顶最大：" + (paixuqucscore - paixuscore) + "ms");
//            System.out.println("==================================");
